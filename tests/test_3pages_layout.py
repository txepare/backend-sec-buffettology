...
def test_render():
    ...
    test_pdf_path = "tests/test_googl_3pages.pdf"
    with PdfPages(test_pdf_path) as pdf:
        show_company_overview(pdf, overview_data, ticker=ticker, market_data=market_data)
        show_revenue_segments_table(pdf, segments_data, ticker=ticker, market_data=market_data)
        show_income_statement_flow(pdf, flow_prepared, ticker=ticker, market_data=market_data)

    print(f"Generated test PDF: {test_pdf_path} (size: {os.path.getsize(test_pdf_path)} bytes)")
